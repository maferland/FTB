  ...
  bool MP3FileInfo::Init(const char* cszFilename)
   {
	...
	sprintf_s(szFilename, strlen(cszFilename)+1, cszFilename); // Modif fait moi même, auparavant sprintf(szFilename, cszFilename)
	...
	if(szYear)  
	{
		sscanf_s(szYear,  "%i", &nYear);
	} //Modifier moi-même, auparavant ; if(szYear)  sscanf(szYear,  "%i", &nYear);
	if(szTrack) 
	{
		sscanf_s(szTrack, "%i", &nTrack);
	} //Modifier moi-même, auparavant ; if(szYear)  sscanf(szYear,  "%i", &nYear);
  ...
  void MP3FileInfo::m_parseGenre(void)
   {
	...
	char buf[10];
	if(!m_getString(ID3FID_CONTENTTYPE, buf, 10))
	 {
	  szGenre = 0;
	  return;
	 }
	...
	int gid;
	sscanf_s(buf, "(%i)", &gid); // filter Genre ID //Modifier moi-même, auparavant ; if(szYear)  sscanf(szYear,  "%i", &nYear);

	if(gid >= ID3_NR_OF_V1_GENRES) // genre ID too large
	 {
	  szGenre = new char[strlen(buf)+1];
	  sprintf_s(szGenre, strlen(buf)+1, buf); //Changer moi-même
	  return;
	 }

	szGenre = new char[strlen(ID3_V1GENRE2DESCRIPTION(gid))+1];
	sprintf_s(szGenre, strlen(ID3_V1GENRE2DESCRIPTION(gid))+1, ID3_V1GENRE2DESCRIPTION(gid)); // Changer moi même
   }
  ...
  void MP3FileInfo::m_parseHeader(void)
   {
	...
	switch(m_mp3header->layer)
	 {
	  case MPEGLAYER_UNDEFINED: sprintf_s(szMpegLayer, 10, "UNDEFINED"); break;
	  case MPEGLAYER_III:       sprintf_s(szMpegLayer, 10, "Layer III"); break;
	  case MPEGLAYER_II:        sprintf_s(szMpegLayer, 10, "Layer II");  break;
	  case MPEGLAYER_I:         sprintf_s(szMpegLayer, 10, "Layer I");   break;
	...
	switch(m_mp3header->version)
	 {
	  case MPEGVERSION_2_5: sprintf_s(szMpegVersion, 10, "MPEG 2.5"); break;
	  case MPEGVERSION_2:   sprintf_s(szMpegVersion, 10, "MPEG 2");   break;
	  case MPEGVERSION_1:   sprintf_s(szMpegVersion, 10, "MPEG 1");   break;
	...
	switch(m_mp3header->channelmode)
	 {
	  case MP3CHANNELMODE_STEREO:         sprintf_s(szChannelMode, 15, "Stereo");         break;
	  case MP3CHANNELMODE_JOINT_STEREO:   sprintf_s(szChannelMode, 15,"Joint Stereo");   break;
	  case MP3CHANNELMODE_DUAL_CHANNEL:   sprintf_s(szChannelMode, 15,"Dual Channel");   break;
	  case MP3CHANNELMODE_SINGLE_CHANNEL: sprintf_s(szChannelMode, 15,"Single Channel"); break;
	...
	/// extracts a string
  bool MP3FileInfo::m_getString(ID3_FrameID fid, char* szBuffer, int length)
   {
	assert(m_bOK);
	...
	if(frame==0)
	 {
	  sprintf_s(szBuffer, length, "[Not Found]");
	  return false;
	 }

	if(!frame->Contains(ID3FN_TEXTENC))
	 {
	  sprintf_s(szBuffer, length, "[Contains No Text]");
	  return false;
	 }
	...
	if(field==0)
	 {
	  delete frame;
	  sprintf_s(szBuffer, length, "[Error]");
	  return false;
	 }
	...
	if(res==0)
	 {
	  delete frame;
	  sprintf_s(szBuffer, length, "[Empty]");
	  return false;
	 }

	sprintf_s(szBuffer, length, res);
	return true;
   }
  ...
  char* MP3FileInfo::m_getNewString(ID3_FrameID fid)
   {
	...
	sprintf_s(buf, strlen(res)+1, res); // copy
